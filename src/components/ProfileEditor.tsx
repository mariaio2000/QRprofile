...
import CardTemplatesStep from "./templates/CardTemplatesStep";
import { CardTemplateId, TemplateOptions } from "./templates/templates";
...
const EMPTY: Profile = {
  ...
  themeFrom: "#4F46E5",
  themeTo: "#6366F1",
  templateId: "photo" as CardTemplateId,
  templateOptions: {
    palette: { from: "#3B82F6", to: "#6366F1" },
    font: "Brand",
    avatar: "circle",
    background: "gradient",
    density: "cozy",
    visible: { phone: true, email: true, socials: true },
  },
};
...
export default function ProfileEditor({ initialProfile, onAutoSave, onBack, onFinish }: Props) {
  ...
  const [cardStep, setCardStep] = useState<{
    ...
    template: profile.templateId || "photo",
    options: profile.templateOptions || {
      palette: { from: "#3B82F6", to: "#6366F1" },
      font: "Brand",
      avatar: "circle",
      background: "gradient",
      ...
      {stepIndex === 3 ? (
        <CardTemplatesStep
          profile={{
            ...
            title: profile.title,
            phone: profile.phone,
            email: profile.website, // Using website as email for now
            socials: profile.socials,
            avatarUrl: profile.profile_image_id ? `data:image/jpeg;base64,${profile.profile_image_id}` : undefined,
          }}
          value={cardStep}
          onChange={setCardStep}
          onBack={() => setStepIndex(2)}
          onSave={async () => {
            // merge chosen template/options into your persisted profile/theme model
            setProfile((p) => ({
              ...p,
              themeFrom: cardStep.options.palette.from,
              themeTo: cardStep.options.palette.to,
              templateId: cardStep.template,
              templateOptions: cardStep.options,
            }));
            await onAutoSave(profile);
          }}
          onFinish={async () => {
            // Reuse your existing publish/onPublish logic
            await onFinish();
          }}
        />
      ) : (
